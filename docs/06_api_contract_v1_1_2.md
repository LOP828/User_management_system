# AI红娘配对跟进管理系统 API契约文档 v1.1.2

## 1. 文档信息

- 文档名称：API契约文档
- 版本：v1.1.2
- 依赖文档：MVP PRD v1.3、业务流程 v1.1.1、数据库设计 v1.1.1、业务规则 v1.1.1、技术选型 v1.1.1
- API风格：RESTful
- Base URL：`/api/v1`
...
| OVERDUE_REASON_REQUIRED | 400 | 处理未首见超时提醒时必须填写超时原因 |
| FORCE_REASON_REQUIRED | 400 | 管理员 force 操作必须填写 force_reason |
...
**返回字段口径：**
- 当请求人为 `matchmaker` 时：返回精简字段，仅包含 `id`、`name`、`role`、`status`
- 当请求人为 `admin` 时：返回完整字段，包含 `id`、`name`、`role`、`phone`、`wechat_id`、`status`

**成功响应（200，matchmaker 视角）：**
```json
{
  "count": 2,
  "results": [
...
**说明：**
- 创建用户时 `paid_at` 可为空，创建接口不要求填写
- 当后续确认用户完成付费时，通过 `PATCH /api/v1/users/{id}/` 写入 `paid_at`
...
**请求体示例二：管理员 force 修改负责人**
```json
{
  "owner_id": 2,
  "force": true,
  "force_reason": "门店负责人调整分配"
}
```

**规则说明：**
- `paid_at` 创建时可为空，后续通过本接口写入；一旦写入，即作为未首见提醒的起算时间
- 红娘不可通过本接口修改 `owner_id`
- 管理员直接修改 `owner_id` 时，必须同时传 `force = true` 和 `force_reason`
- 管理员 force 修改 `owner_id` 后，需同步更新进行中的配对卡红娘字段、该用户 pending 提醒的 `staff_id`，并更新 `user.last_action_at`
- 管理员未传 `force_reason` → 400 `FORCE_REASON_REQUIRED`
...
**规则说明：**
- `force` 默认 `false`
- 仅 admin 可传 `force = true`
- `force = true` 时可跳过普通白名单状态流转校验，但 `force_reason` 必填
- 红娘传 `force = true` → 403 `PERMISSION_DENIED`
...
**失败响应（400）：** USER_STATUS_TRANSITION_INVALID / FORCE_REASON_REQUIRED。
...
**说明：** `reason_id` / `reason_note` 同步写入 `user_status_history.reason_id` / `user_status_history.reason_note`。
...
**字段说明：**
- `last_visit_at`：双方最近一次任一单方有效回访时间的最大值
- `next_remind_at`：当前该配对卡所有未完成回访提醒中最早到期的时间
...
| overdue_reason_id | int | 否 | reason_enum.id | 仅 first_meet_overdue 提醒闭环生成的 unmatched 跟进记录使用 |
| overdue_reason_note | string | 否 | — | 超时原因补充说明 |

**校验规则：**
- scene = "matched" 时，`is_still_contact`、`risk_status`、`next_remind_mode` 缺一不可，缺失任一字段 → 400 VALIDATION_ERROR
...
**查看范围：**
- `matchmaker`：默认仅查看自己的提醒，后端自动按当前登录人的 `staff_id` 过滤
- `admin`：默认查看全局提醒，可选按 `staff_id` 过滤某位红娘的提醒
...
{
  "overdue_reason_id": 3,
  "overdue_reason_note": "近期库存不足，已重新安排推荐计划"
}
```

**规则说明：**
- 普通提醒可传空对象 `{}` 或不传请求体
- 当 `remind_type = "first_meet_overdue"` 时，`overdue_reason_id` 必填，`overdue_reason_note` 选填
- 处理 `first_meet_overdue` 时，后端必须自动新增一条 `scene = "unmatched"` 的跟进记录，并将 `overdue_reason_id` / `overdue_reason_note` 落到该 `follow_up_record`
- 提醒处理成功后，需同步更新 `last_action_at`：`target_type = "user"` 更新该用户；`target_type = "match_card"` 更新配对卡双方用户
- 仅把提醒改成 `processed`，但未写入上述跟进记录，不算完成闭环
...
| 版本 | 日期 | 修改内容 |
|-----|------|---------|
| v1.1.2 | 2026-03-14 | 明确 GET /staff/ 的 admin 与 matchmaker 返回字段差异，并统一 GET /reminders/ 的查看范围与 staff_id 过滤口径 |
| v1.1.1 | 2026-03-14 | 增补 reminders/process 处理未首见超时的闭环请求体，补充 paid_at 写入规则、用户接口 force 能力和配对卡回访字段语义 |
| v1.0 | — | 初始版本，38个接口 |

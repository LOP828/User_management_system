# AI红娘配对跟进管理系统 权限矩阵文档 v1.1.2

## 1. 文档信息

- 文档名称：权限矩阵文档
- 版本：v1.1.2
- 依赖文档：MVP PRD v1.3、业务规则 v1.1.1、API契约 v1.1.2
...
|------|------|------|------|-------|---------|
| 当前登录人 | GET | /staff/me/ | ✅ | ✅ | — |
| 红娘列表 | GET | /staff/ | ✅ | ✅ | 红娘：仅返回 `id/name/role/status`；管理员：返回 `id/name/role/phone/wechat_id/status` |
...
| 接口 | 方法 | 路径 | 红娘 | 管理员 | 附加条件 |
|------|------|------|------|-------|---------|
| 我的提醒 | GET | /reminders/ | ✅ 自己的 | ✅ 全部 | 红娘：自动注入 staff_id = 自己；管理员：默认全局可见，可按 `staff_id` 过滤 |
...
| ReminderListView | [IsAuthenticated] （matchmaker 自动按当前 staff_id 过滤；admin 默认全量，可按 staff_id 过滤） |
...
| 版本 | 日期 | 修改内容 |
|-----|------|---------|
| v1.1.2 | 2026-03-14 | 明确 GET /staff/ 的角色差异返回字段，并统一 GET /reminders/ 的 matchmaker/admin 查看范围与过滤规则 |

# AI红娘配对跟进管理系统 权限矩阵文档 v1.1

## 1. 文档信息

- 文档名称：权限矩阵文档
- 版本：v1.1
- 依赖文档：MVP PRD v1.3、业务规则 v1.1、API契约 v1.1
...
| 用户详情 | GET | /users/{id}/ | ✅ 有关联 | ✅ | 红娘：owner 或涉及自己的配对卡中的用户 |
| 编辑用户 | PATCH | /users/{id}/ | ✅ owner | ✅ | 红娘：仅 owner_id = 自己 |
| 状态变更 | POST | /users/{id}/change-status/ | ✅ owner | ✅ | 红娘：限合法流转路径（BR-POOL-001）；管理员：不限 |
...
| 写跟进 | ✅ owner | ✅ | 始终显示 | — |
| 设置提醒 | ✅ owner | ✅ | 始终显示 | — |
| 强制修改状态 | ❌ | ✅ | 始终显示（仅管理员可见） | 不受状态流转白名单限制 |
| 强制修改负责人 | ❌ | ✅ | 始终显示（仅管理员可见） | — |
...
class CanViewUser(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        return MatchCard.objects.filter(
            ...
            stage__in=['initial_contact', 'stable_contact', 'success_pending_review']
        ).exists()
```
...
| UserUpdateView | [IsAuthenticated, IsUserOwner] |

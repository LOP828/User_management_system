# AI红娘配对跟进管理系统 业务规则文档 v1.1.1

## 1. 文档信息

- 文档名称：业务规则文档
- 版本：v1.1.1
- 依赖文档：MVP PRD v1.3、业务流程文档 v1.1.1、数据库设计文档 v1.1.1
...
**字段语义：**
- user.is_in_match = TRUE 仅表示存在上述进行中的配对卡
- 当 match_card.stage = 'success' 或 'ended' 时，user.is_in_match = FALSE
...
### BR-USER-006：paid_at 写入规则

**触发时机：** 用户创建、用户资料编辑保存时

**规则：**
- 创建用户时 paid_at 可为空
- 当确认用户已付费完成时，由 PATCH /users/{id}/ 写入 paid_at
- paid_at 一旦写入，即作为未首见提醒（BR-REMIND-001）的唯一起算时间
- 若 paid_at 为空，则不参与未首见提醒计算

### BR-USER-007：last_action_at 更新规则

**触发时机：** 以下任一有效动作完成时

**规则：** 系统更新 user.last_action_at = 当前时间
...
3. 用户状态变更
4. 创建配对卡
5. 更新配对卡关键字段（male_feedback、female_feedback、male_heat、female_heat、staff_judgment、risk_level、risk_reason_id、risk_reason_note）
6. 处理提醒
7. 转移负责人（审批通过或管理员 force 修改）

**处理提醒时的更新对象：**
- 若 reminder.target_type = 'user'，更新该 user.last_action_at
- 若 reminder.target_type = 'match_card'，更新配对卡双方用户的 user.last_action_at
...
### BR-POOL-007：管理员 force 状态变更规则

**触发时机：** 管理员通过 POST /users/{id}/change-status/ 传 force = TRUE

**规则：**
1. 仅 admin 可传 force = TRUE
2. force = TRUE 时，允许跳过 BR-POOL-001 的普通白名单流转校验
3. force_reason 必填
4. 仍需校验目标状态值本身合法（必须是 user.pool_status 支持的枚举值）
5. 写入 user_status_history.reason_id = NULL，user_status_history.reason_note = force_reason
6. 写入 operation_log.action = 'admin_force_change'，operation_log.reason = force_reason
...
- 男方 is_in_match = TRUE
- 女方 is_in_match = TRUE
- 男方 user.last_action_at = 当前时间
- 女方 user.last_action_at = 当前时间
...
### BR-MATCH-009：配对卡关键字段更新时间规则

**触发时机：** 通过 PATCH /match-cards/{id}/ 或 POST /match-cards/{id}/set-risk/ 更新关键字段时

**规则：**
1. 更新双方用户的 user.last_action_at = 当前时间
2. risk_level / risk_reason_id / risk_reason_note 变更时，同步更新双方用户的 user.last_action_at
3. 仅统计以下字段为"关键字段"：male_feedback、female_feedback、male_heat、female_heat、staff_judgment、risk_level、risk_reason_id、risk_reason_note
...
- overdue_reason_id（仅处理 first_meet_overdue 提醒时必填，通常由提醒处理接口代填）
- overdue_reason_note（仅处理 first_meet_overdue 提醒时可填）
...
**保存后联动：**
- 更新 match_card.last_visit_at = 男方/女方最近一次任一单方有效回访时间的最大值
...
- 更新 match_card.next_remind_at = 当前未完成回访提醒中最早到期时间
...
**T+5 超时特殊规则：**
- 生成 first_meet_overdue 提醒后，红娘必须通过 POST /reminders/{id}/process/ 提交 overdue_reason_id，overdue_reason_note 选填
- 系统在处理该提醒时，必须自动新增一条 scene = 'unmatched' 的 follow_up_record，并写入 overdue_reason_id / overdue_reason_note
- 仅将提醒标记为 processed，不自动写入 follow_up_record，不算完成闭环
...
**字段维护：**
- match_card.next_remind_at = 当前该配对卡所有未完成回访提醒中最早到期的 remind_at
...
**补充规则：**
- 当 remind_type = 'first_meet_overdue' 时，processed 必须伴随一条新增的 scene = 'unmatched' 跟进记录，且该记录 overdue_reason_id 非空
...
6. 写入操作日志
7. 将男方、女方 user.is_in_match 统一置为 FALSE
...
7. 写入操作日志
8. 男方、女方 user.is_in_match 保持 FALSE
...
6. 通知新负责人（微信消息）
7. 写入操作日志
8. 更新 user.last_action_at = 当前时间

### BR-TRANSFER-003：管理员 force 修改负责人规则

**触发时机：** 管理员通过 PATCH /users/{id}/ 传 owner_id 且 force = TRUE

**规则：**
1. 仅 admin 可直接修改 owner_id
2. 直接修改 owner_id 时，force = TRUE 且 force_reason 必填
3. 修改后执行与 BR-TRANSFER-002 相同的联动：
   - 更新 user.owner_id
   - 更新进行中的配对卡红娘字段
   - 更新该用户所有 pending 提醒的 staff_id
   - 更新 user.last_action_at = 当前时间
4. 写入 operation_log.action = 'admin_force_change'，operation_log.reason = force_reason
...
| 版本 | 日期 | 修改内容 |
|-----|------|---------|
| v1.1.1 | 2026-03-14 | 修复 overdue 原因落点、管理员 force 能力、paid_at / last_action_at / is_in_match / 配对卡回访字段语义，并统一 met_not_continue 两步流转口径 |

# AI红娘配对跟进管理系统 数据库设计文档 v1.1

## 1. 文档信息

- 文档名称：数据库设计文档
- 版本：v1.1
- 依赖文档：MVP PRD v1.3、业务流程与状态流转文档 v1.1
...
| is_in_match | BOOLEAN | 是 | FALSE | 是否在已配对池中（有进行中的配对卡） |
| paid_at | TIMESTAMP | 否 | NULL | 付费时间（T+N 的起算点） |
| last_action_at | TIMESTAMP | 否 | NULL | 最近有效动作时间（管理员用） |
...
| last_visit_at | TIMESTAMP | 否 | NULL | 最近回访时间 |
| next_remind_at | TIMESTAMP | 否 | NULL | 下次提醒时间 |
...
| 约束 | 实现层 | 说明 |
|------|-------|------|
| 用户同一时间只能有一张进行中配对卡 | 应用层 | 创建配对卡时校验 is_in_match = FALSE |
